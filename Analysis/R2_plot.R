...
SpComp_R2 <-read_csv("Results/SpComp_R2.csv")%>% dplyr::select(Variables, R2) %>% rename(SpComp=R2)
...
R2_all <- Abund_R2 %>% 
  ...
  full_join(SpComp_R2, by="Variables") %>% 
...
ggplot(R2, aes(x = Diversity, y = Drivers, size = R2, colour = Drivers, fill = Drivers)) +
 ...
 # scale_x_discrete(position = "top") +
  scale_size_continuous(range = c(0.5, 27)) + 
